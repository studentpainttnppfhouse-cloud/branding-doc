async function fetchShopifyOrders({ storeUrl, accessToken, fulfillmentStatus = 'unfulfilled', limit = 250 }) {
...
function normalizeShopifyCSV(rows) {
  const map = new Map()
  ...
  for (const row of rows) {
    ...
    if (!map.has(orderNum)) {
      map.set(orderNum, {
        ...
        customerName:      row['Billing Name'] || row['billing_name'] || row['Customer Name'] || '',
        ...
        shippingAddress: [
          row['Shipping Address1'], row['Shipping City'],
          row['Shipping Province'],  row['Shipping Zip'], row['Shipping Country'],
        ].filter(Boolean).join(', '),
      })
    }
  ...
  return Array.from(map.values()).map(o => ({ ...o, lineItems: o.lineItems.join(', ') }))
}

module.exports = { fetchShopifyOrders, normalizeShopifyCSV }

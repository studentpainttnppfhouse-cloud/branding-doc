...
/**
 * Send delivery proof email to the customer.
 * Attaches all media files and embeds a link to a public proof page if available.
 */
export async function sendProofEmail({ order, mediaList, baseUrl }) {
  const transporter = createTransport()

  const attachments = mediaList.map(media => {
    const filePath = path.join(process.cwd(), '.data', 'media', media.filename)
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)">
        ...
        <tr><td style="background:#0369a1;padding:28px 32px">
          <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700">📦 Delivery Proof</h1>
        ...
        <tr><td style="padding:32px">
          ...
          <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px;margin:20px 0">
            <table width="100%" cellpadding="6">
              <tr><td style="color:#64748b;font-size:13px;width:120px">Order</td><td style="font-weight:600;color:#0f172a">${order.orderNumber}</td></tr>
          ...
          <ul style="margin:0 0 24px;padding-left:20px;color:#475569;font-size:14px">
            ${videoAttachments.length ? `<li>🎬 ${videoAttachments.length} video recording${videoAttachments.length !== 1 ? 's' : ''}</li>` : ''}
            ${imageAttachments.length ? `<li>📸 ${imageAttachments.length} photo${imageAttachments.length !== 1 ? 's' : ''}</li>` : ''}
          </ul>

          <p style="color:#475569;font-size:14px;line-height:1.6">
            If you have any concerns about your order, please reply to this email before we dispatch.
            Once satisfied, your items will be shipped shortly.
          </p>
        </td></tr>
        <!-- Footer -->
